...
export function Achievement({
  ...
  const interventionAssessments = completed.filter((item) => item.interventionCount > 0).length;
  ...
  return (
    <div className="page achievement-page">
      ...
      <section className="achievement-metrics">
        ...
        <AchievementMetric label="تقويمات مكتملة" value={completed.length} detail="سجلات غير مسودة وبها طلبة" />
        <AchievementMetric label="بها فئة مستهدفة بالتدخل" value={interventionAssessments} detail="وفق التصنيف والمرجع المسجل لكل تقويم" danger={interventionAssessments > 0} />
...
function AssessmentCard({ assessment, onOpen }: { assessment: AchievementAssessmentRecord; onOpen: () => void }) {
  ...
  return (
    <button className={`assessment-card ${assessment.status !== 'draft' && assessment.interventionCount > 0 ? 'needs-attention' : ''}`} onClick={onOpen}>
...
function AssessmentDetails({ assessment, teachers, onReload }: { assessment: AchievementAssessmentDetails; teachers: Teacher[]; onReload: () => Promise<void> }) {
  ...
  return <div className="achievement-detail">
    <div className="achievement-detail-hero">
      <div><span className="eyebrow">{assessment.subject} • {assessment.grade}</span><h2>{assessment.title}</h2><p>{assessment.assessmentType} • {assessment.academicYear} • {formatDate(assessment.assessmentDate)} • {assessment.teacherName || 'دون معلم مسؤول'}</p></div>
    ...
    {editing ? <AssessmentForm assessment={assessment} teachers={teachers} onSubmit={saveAssessment} busy={busy}/> : <>
      <section className="achievement-detail-metrics">
        <AchievementMetric label="الفئة المحققة للحد" value={`${assessment.masteryPercent}%`} detail={`وفق الحد المسجل ${assessment.masteryThresholdPct}%`} danger={assessment.interventionCount > 0}/>
...
export function AssessmentModal({ open, teachers, academicYear, term, onClose, onCreated }: { open: boolean; teachers: Teacher[]; academicYear: string; term: string; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <Modal open={open} onClose={onClose}><form className="request-form" onSubmit={submit}><div className="modal-heading"><span className="eyebrow">سجل تحصيل جديد</span><h2>تسجيل نتيجة وتقسيم الأداء</h2><p>سجل المؤشرات الأساسية فقط. التحليل المتقدم سيظل في محركاته المتخصصة ثم يرتبط بهذا السجل.</p></div><AssessmentFields teachers={teachers} defaultAcademicYear={academicYear} defaultTerm={term}/>{message&&<div className="inline-error"><Icon name="alert" size={17}/>{message}</div>}<div className="modal-actions"><button type="button" className="ghost-button" onClick={onClose}>إلغاء</button><button className="primary-button" disabled={saving}>{saving?'جاري الحفظ...':'تسجيل النتيجة'}</button></div></form></Modal>;
}
