...
export function DocumentsWorkspace({
  ...
  if (loading) return <State title="جاري تحميل سجل الوثائق من Supabase" detail="قراءة بيانات الوثائق عبر RLS." busy/>;
  ...
  async function openDocument(document: DocumentRecord): Promise<void> {
    if (!context) return;
    try {
      const signedUrl = await createSupabaseDocumentSignedUrl(context, document);
      window.open(signedUrl, '_blank', 'noopener,noreferrer');
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'تعذر فتح الوثيقة.');
    }
  }
