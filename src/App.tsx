...
function AdminApp(){
  ...
  function action(name:string){setQuick(false);if(name==='request'){setRequestModal(true);return;}if(name==='event'){setEventModal(true);return;}setToast('تم تثبيت هذا الإجراء في بنية التطبيق.');}
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <section className="workspace">{view==='dashboard'?<Dashboard data={data} onQuickAction={action}/>:view==='teachers'?<Teachers teachers={data.teachers} onAddTeacher={()=>setTeacherModal(true)}/>:view==='requests'?<Requests requests={data.requests} onNewRequest={()=>setRequestModal(true)} onStatus={changeStatus}/>:view==='events'?<Events events={data.events} onAddEvent={()=>setEventModal(true)}/>:view==='documents'?<Documents documents={data.documents}/>:<Placeholder view={view}/>}</section>
    </main>
    ...
    <EventModal open={eventModal} onClose={()=>setEventModal(false)} onCreated={async()=>{setEventModal(false);setToast('تم توثيق الفعالية');await refresh();}}/>
...
function EventModal({open,onClose,onCreated}:{open:boolean;onClose:()=>void;onCreated:()=>Promise<void>}){
  const [saving,setSaving]=useState(false);const [message,setMessage]=useState('');
  async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const formElement=event.currentTarget;setSaving(true);setMessage('');const form=new FormData(formElement);const payload:CreateEventInput={title:String(form.get('title')),eventType:String(form.get('eventType')),eventDate:String(form.get('eventDate')),location:String(form.get('location')||''),audience:String(form.get('audience')||''),participantCount:Number(form.get('participantCount')||0),goals:String(form.get('goals')||''),summary:String(form.get('summary')||''),outcomes:String(form.get('outcomes')||''),recommendations:String(form.get('recommendations')||'')};try{await createEvent(payload);formElement.reset();await onCreated();}catch(e){setMessage(e instanceof Error?e.message:'تعذر توثيق الفعالية.');}finally{setSaving(false)}}
  return <Modal open={open} onClose={onClose}><form className="request-form" onSubmit={submit}><div className="modal-heading"><span className="eyebrow">سجل توثيق</span><h2>توثيق فعالية</h2><p>نسجل جوهر الفعالية تربويًا أولًا. الصور والأدلة ترتبط بالسجل نفسه بدل أن تعيش في مجلد مجهول.</p></div><div className="form-grid"><label className="full">عنوان الفعالية<input name="title" required placeholder="مثال: أسبوع العلوم"/></label><label>نوع الفعالية<select name="eventType"><option>فعالية</option><option>مسابقة</option><option>مبادرة</option><option>زيارة علمية</option><option>مشروع طلابي</option></select></label><label>التاريخ<input type="date" name="eventDate" required defaultValue="2026-09-01"/></label><label>المكان<input name="location" placeholder="المدرسة / القاعة / ..."/></label><label>الفئة المستهدفة<input name="audience" placeholder="الصف العاشر مثلاً"/></label><label>عدد المشاركين<input type="number" min="0" name="participantCount" defaultValue="0"/></label><label className="full">الأهداف<textarea name="goals" rows={2} placeholder="الأهداف التربوية للفعالية"/></label><label className="full">ملخص التنفيذ<textarea name="summary" rows={3} placeholder="ماذا نُفذ وكيف؟"/></label><label className="full">النتائج والمخرجات<textarea name="outcomes" rows={2}/></label><label className="full">التوصيات<textarea name="recommendations" rows={2}/></label></div>{message&&<div className="inline-error"><Icon name="alert" size={17}/>{message}</div>}<div className="modal-actions"><button type="button" className="ghost-button" onClick={onClose}>إلغاء</button><button className="primary-button" disabled={saving}>{saving?'جاري الحفظ...':'حفظ التوثيق'}</button></div></form></Modal>
}
...
function Placeholder({view}:{view:View}){const map:Record<string,[string,string,string[]]>={planning:['التخطيط والمنهج','متابعة الخطة السنوية والفصلية وتنفيذ المنهج دون تحويل الشاشة إلى جدول ممتد حتى ولاية أخرى.',['الخطة السنوية','تنفيذ المنهج','التأخير والمعالجة']],achievement:['التحصيل والنتائج','مسار واضح من النتائج إلى التشخيص والتدخل وقياس الأثر.',['النتائج','التحليل','البرامج العلاجية']],supervision:['الإشراف والمتابعة','خطة زيارات وتوصيات ودعم ومتابعة مرتبطة بملف المعلم.',['الزيارات','الدعم','المتابعة']],meetings:['الاجتماعات والقرارات','كل قرار يتحول إلى مهمة لها مسؤول وموعد وحالة.',['المحاضر','القرارات','المهام']],reports:['مركز التقارير','تجميع تقارير المادة والفصل والفعاليات وملف الإنجاز في مركز واحد.',['تقرير الفصل','ملف الإنجاز','التسليم والاستلام']],archive:['الأرشيف التاريخي','ذاكرة المادة عبر السنوات بدون خلط الماضي بالحاضر.',['2026/2027','2025/2026','مقارنة الأعوام']]};const item=map[view]||['قريبًا','هذه المساحة ضمن الهيكل الأساسي.',['جاهزة للتوسعة']];return <div className="placeholder"><span className="placeholder-icon"><Icon name="spark" size={30}/></span><span className="eyebrow">ضمن النواة</span><h1>{item[0]}</h1><p>{item[1]}</p><div className="tag-row">{item[2].map(x=><span key={x}>{x}</span>)}</div></div>}
function Loading(){return <div className="loading-screen"><span className="brand-mark large">ق</span><div className="spinner"></div><p>جاري تجهيز مساحة المادة...</p></div>}
