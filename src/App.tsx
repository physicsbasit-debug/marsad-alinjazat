...
function AdminApp(){
  ...
  function action(name:string){setQuick(false);if(name==='request'){if(data&&data.academicYear!==data.currentAcademicYear){setToast('طلبات الملفات تشغيلية للعام الجاري. أدخل السجلات التاريخية من وحداتها الأصلية.');return;}if(data&&REQUESTS_DOCUMENTS_DATA_MODE==='supabase'&&data.academicYear===data.currentAcademicYear){setView('requests');setToast('إنشاء رابط الرفع سينتقل في مرحلة الرفع العام؛ المراجعة الحالية تعمل من Supabase.');return;}setRequestModal(true);return;}if(name==='event'){setEventModal(true);return;}if(name==='meeting'){setMeetingModal(true);return;}if(name==='visit'){if(data&&SUPERVISION_DATA_MODE==='supabase'&&data.academicYear===data.currentAcademicYear){setView('supervision');setSupervisionCreateSignal((value)=>value+1);return;}setVisitModal(true);return;}if(name==='assessment'){setAssessmentModal(true);return;}setToast('تم تثبيت هذا الإجراء في بنية التطبيق.');}
