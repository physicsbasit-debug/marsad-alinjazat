...
الإصدار الحالي: **v0.24.1** (تصحيح Fixture اختبار القبول)  
...
أثناء القبول الحي لـv0.24.0 ظهر أن اختبار `SET NULL` كان يترك مستندًا مؤقتًا ثالثًا مرتبطًا بالمعلم داخل نفس Transaction. لم يكن ذلك تسرب RLS؛ بل جعل عدّ مستندات المعلم اللاحق يساوي 3 بدل المستندين الأساسيين. في v0.24.1 يُحذف ذلك المستند المؤقت مباشرة بعد إثبات `request_id IS NULL`، ثم تستمر اختبارات الأدوار على الـfixture الأساسية. لا توجد أي Migration أو تغييرات Schema/RLS.
